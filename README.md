...
[`v0.2.2`](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.2.2) 在不改变研究数据与 schema 的前提下，减少二级导航复杂度，把状态视图整合为页面内筛选，简化 Topbar 与工作区设置，并为路由、弹窗、抽屉和状态切换加入统一、克制的界面动效。系统完整尊重 `prefers-reduced-motion`，且未增加动画依赖。
...
`0.1` 系列建立核心研究生命周期、安全导入导出、质量门禁和公开维护基础；`v0.2.x` 已发布双语工作站、加密工作台、Theory、简化导航与 PWA。已授权的 `v0.3.0` 非地图工作已在 `main` 完成：可折叠活动导航、Zotero Handoff/插件、导入防护、浏览器 E2E、安全自动化与隐私安全诊断均已合并。但 2026-08-15 发布时复核仍把省级中国田野地图的来源、再分发、审图元数据和全国完整性四项门禁全部判为 BLOCKED；由于本轮冻结范围把地图设为必选项，`v0.3.0` 处于 BLOCKED / NOT RELEASED，没有 tag 或 GitHub Release。
