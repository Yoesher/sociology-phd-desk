...
The current public deployment remains under `yoesher.github.io`, where IndexedDB and service-worker authority are isolated by origin rather than repository path. See [the canonical-origin decision](docs/architecture/canonical-origin.md). A future origin cannot automatically read this origin's IndexedDB; transfer must use an explicit user-controlled encrypted backup and retain an old-site migration notice.
