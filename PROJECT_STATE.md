...
> Status: [`v0.2.2`](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.2.2) remains the latest formal Release while `release/0.3.0` is prepared from exact verified main `79d2d5a1df0eb578b163dd78c50f5cece267d653`. All authorized non-map `v0.3.0` work is merged: navigation collapse, Zotero/schema v5, plugin 0.1.0, guarded imports, browser E2E, security automation, and privacy-safe diagnostics/feedback metadata. The China map is formally DEFERRED and excluded from this release; its blocked compliance evidence remains preserved. The release PR, tag, GitHub Release, public XPI, and exact release SHA are still PENDING and must not be claimed before verification.
...
- Current target: finalize and release `v0.3.0` without the map. Package/lock/CITATION advance to `0.3.0`; portable/standard remain v5 and encrypted container remains v1. The map's source/license/approval/completeness evidence stays BLOCKED, but the map itself is excluded and no longer blocks the non-map release.
...
### v0.3.0 completed delivery chain and release candidate — through 2026-08-15
...
- Zotero plugin 0.1.0 was tested with synthetic metadata in isolated Zotero 8 and Zotero 9 profiles. The exact release candidate reproducibly emits a 13,120-byte XPI with SHA-256 `e940f29bb803774a9311b0b5c8f40776558c9362bfa58f28c01681e5ed7795ee`; two independent builds and the generated checksum file match. This intentionally differs from the earlier `0a0ec706…` candidate because the packaged plugin README now replaces the obsolete “v0.3.0 blocked” statement with the verified release-asset boundary. No real Zotero library, profile, account, attachments, notes, annotations, or sync state was used.
...
### v0.3.0 local release-candidate gate — 2026-08-15
...
| Release publication | PENDING — release PR, exact-head CI/CodeQL/self-review, merge, exact-main CI/CodeQL/Pages, public smoke, annotated tag, UTF-8 Release, public assets, and downloaded-XPI verification remain required |
...
- `release/0.3.0` now exists as a local release candidate from exact verified main. No annotated `v0.3.0` tag, GitHub Release, or public plugin asset exists yet; those remain gated on local validation, PR CI/self-review, exact-main CI/CodeQL/Pages, and release-asset verification.
...
Complete the `v0.3.0` release chain without the map, then stop at `V0.3.0 ADOPTION & EXTERNAL TESTING`; do not start `v0.4`. The Phase 3D evidence remains binding for any future map work: all four gates are **BLOCKED**, no map code/geometry/API/catalog/region persistence or participant GPS ships, and Issues #8/#37 remain closed as `not planned`. Issue [#2](https://github.com/Yoesher/sociology-phd-desk/issues/2) remains separate, OPEN, and unimplemented. Do not translate user-authored research content or claim external adoption without evidence.
