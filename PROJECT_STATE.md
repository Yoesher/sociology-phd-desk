...
> Status: [`v0.3.0`](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.3.0) is the latest formal Release at exact release SHA `bb0d32fe99348204ba89a16d6469014ae38e0ecf`. It includes the complete authorized non-map scope: navigation collapse, Zotero/schema v5, plugin 0.1.0, guarded imports, browser E2E, security automation, and privacy-safe diagnostics/feedback metadata. The China map is formally DEFERRED and excluded; its blocked compliance evidence remains preserved.

> Release evidence: [PR #53](https://github.com/Yoesher/sociology-phd-desk/pull/53) exact head `198d8de6562da56ac4f3c7df02265e63541ed5ad` / tree `766c6e52b981d87f5444a5db29741572f2cd9404` passed push CI `31890664277`, PR CI `31890826295`, CodeQL `31890826277`, and [P0 = 0 / P1 = 0 self-review](https://github.com/Yoesher/sociology-phd-desk/pull/53#issuecomment-5302756866), then squash-merged as `bb0d32fe99348204ba89a16d6469014ae38e0ecf` with the reviewed tree unchanged. Exact-main CI `31891195542` / job `95027604928`, CodeQL `31891195526` / job `95027605007`, Pages `31891195535` / build `95027604851` / deploy `95027757672`, and deployment `5921587315` passed. Public root, manifest, service worker, and exact entry bundle returned HTTP 200. Annotated tag object `819d721dd01784f8d40dd001c47c8afd79d8adc8` dereferences to that SHA. Release `371077487` is latest, non-draft, non-prerelease; UTF-8 API read-back and public Zotero asset checksum verification passed.
...
- Package/lock and published release: `0.3.0`; exact release SHA `bb0d32fe99348204ba89a16d6469014ae38e0ecf`
- Release status: [`v0.3.0`](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.3.0) is latest, non-draft, and non-prerelease; annotated tag object `819d721dd01784f8d40dd001c47c8afd79d8adc8` dereferences to the exact release SHA. Existing tags remain unmoved.
- Current target: `V0.3.0 ADOPTION & EXTERNAL TESTING`. Portable/standard are v5 and encrypted container remains v1. The map's source/license/approval/completeness evidence stays BLOCKED; the map is excluded, and no v0.4 work starts in this closeout.
...
### v0.3.0 completed delivery chain and formal release — through 2026-08-15
...
- Zotero plugin 0.1.0 was tested with synthetic metadata in isolated Zotero 8 and Zotero 9 profiles. The exact release source reproducibly emits a 13,120-byte XPI with SHA-256 `e940f29bb803774a9311b0b5c8f40776558c9362bfa58f28c01681e5ed7795ee`; two independent builds, the generated checksum, and a fresh public-asset download match. This intentionally differs from the earlier `0a0ec706…` candidate because the packaged plugin README replaced the obsolete “v0.3.0 blocked” statement with the verified release-asset boundary. No real Zotero library, profile, account, attachments, notes, annotations, or sync state was used.
...
### v0.3.0 final release gate — 2026-08-15

| Gate | Result |
| --- | --- |
| Dependency and static checks | PASS — `npm ci` added 442 locked packages and audited 443; lint 0 warnings; typecheck PASS; `git diff --check` PASS; official-registry high/critical audit found 0 vulnerabilities |
| Unit/integration tests | PASS — 37 files / 328 tests |
| Production/PWA build | PASS — Vite 8.2.1 transformed 1,977 modules; CSS 99.91 kB / 17.78 gzip; main 284.37 / 67.52; i18n 204.04 / 52.17; vendor 409.01 / 128.05; 25 static precache entries / 1,430.54 KiB; static-only PWA contract PASS |
| Browser E2E | PASS — 9 passed / 7 project-configured skips across desktop Chromium and 390 × 844; covered critical CRUD, write-free import preflight, stale-tab conflict, nested dialogs, encrypted lock/reopen, and offline startup/online recovery |
| Zotero plugin | PASS — 8/8 Node tests; deterministic 13,120-byte XPI; independent SHA-256 and checksum agree at `e940f29bb803774a9311b0b5c8f40776558c9362bfa58f28c01681e5ed7795ee` |
| Migration | PASS — the full suite covers explicit v1 → v2 → v3 → v4 → v5, direct legacy upgrades, fresh/roundtrip v5, and authenticated encrypted legacy payload/backup migration without map placeholders |
| Real local browser | PASS with explicit boundaries — 1280 × 720 and 390 × 844, zh-CN/en, active collapse/reopen, Literature/Zotero entry, Projects, Theory, Fieldwork, Publishing, no horizontal overflow, and application console error/warning count 0. A waiting update from the previous 0.2.2 local shell was user-approved while the standard Demo workspace was open; it reloaded to 0.3.0, preserved the workspace, showed the once-per-version summary, and reported portable/database/container 5/5/1. Encrypted/offline behavior is covered by the passing browser E2E; a separate OS-installed PWA launch was NOT RUN because the in-app browser exposes no operating-system install surface. No new QA workspace or real Zotero data was created. |
| Release publication | PASS — PR #53 exact-head CI/CodeQL/self-review, reviewed-tree merge, exact-main CI/CodeQL/Pages/deployment, LIMITED PASS public smoke, annotated tag, UTF-8 Release, both public Zotero assets, and downloaded-XPI checksum verification completed |
...
The following bullets describe released `v0.3.0` at exact verified release revision `bb0d32fe99348204ba89a16d6469014ae38e0ecf`, retaining the v0.2 architecture and adding the non-map v0.3 maintenance/Zotero scope.
...
- Current GitHub release: [Sociology PhD Desk v0.3.0](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.3.0), latest, public, not a draft, and not a prerelease.
- Release assets are public: [Zotero plugin 0.1.0](https://github.com/Yoesher/sociology-phd-desk/releases/download/v0.3.0/sociology-phd-desk-zotero-0.1.0.xpi) and its [SHA-256 file](https://github.com/Yoesher/sociology-phd-desk/releases/download/v0.3.0/sociology-phd-desk-zotero-0.1.0.sha256). The public download matched `e940f29bb803774a9311b0b5c8f40776558c9362bfa58f28c01681e5ed7795ee`.
- Source-backed publication snapshot on 2026-08-15 after PR #53 merged and Release `371077487` published: 3 Stars, 0 Forks, 3 open Issues, 17 closed Issues, 3 open Pull Requests, 30 merged Pull Requests, and 5 published Releases. All three open Pull Requests are automated Dependabot updates. Stars and automated Pull Requests are not evidence of users, testers, endorsements, or adoption.
...
`NEXT_STAGE = V0.3.0 ADOPTION & EXTERNAL TESTING`. Do not start `v0.4`. Seek voluntary, consented researcher feedback without telemetry or automatic tracking. The Phase 3D evidence remains binding for any future map work: all four gates are **BLOCKED**, no map code/geometry/API/catalog/region persistence or participant GPS ships, and Issues #8/#37 remain closed as `not planned`. Issue [#2](https://github.com/Yoesher/sociology-phd-desk/issues/2) remains separate, OPEN, and unimplemented. Do not translate user-authored research content or claim external adoption without evidence.
