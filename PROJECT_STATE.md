...
> Status: [`v0.2.2`](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.2.2) remains the latest formal Release. The authorized `v0.3.0` cycle is active: navigation, app-side Zotero/schema v5, Zotero plugin 0.1.0, guarded imports, browser E2E, and security/dependency automation are merged and exact-main verified through `a8ce50d55ce826da751ddf856f73c31f2cc0a8a1`. Issue #45 diagnostics/feedback/maintainer metadata is a local candidate; Issue #37's release-time map gate remains pending. No `v0.3.0` Release is claimed.
...
- Current target: authorized `v0.3.0` minor-release cycle, strictly sequenced from Phase A navigation collapse through Zotero, guarded imports/schema v5, map compliance, E2E/maintenance, and only then a release candidate. `v0.2.2` remains the latest formal Release.
...
### v0.3.0 completed delivery chain and active maintenance — 2026-08-14

- Milestone `v0.3.0` exists. Issues [#34](https://github.com/Yoesher/sociology-phd-desk/issues/34), [#35](https://github.com/Yoesher/sociology-phd-desk/issues/35), [#36](https://github.com/Yoesher/sociology-phd-desk/issues/36), [#5](https://github.com/Yoesher/sociology-phd-desk/issues/5), [#6](https://github.com/Yoesher/sociology-phd-desk/issues/6), and [#44](https://github.com/Yoesher/sociology-phd-desk/issues/44) are closed as completed; [#37](https://github.com/Yoesher/sociology-phd-desk/issues/37) and [#45](https://github.com/Yoesher/sociology-phd-desk/issues/45) track the map gate and remaining diagnostics/feedback slice. Issues #2/#3/#4 remain future backlog.
- Navigation PR #38, app-side Zotero/schema-v5 PR #39, Zotero plugin PR #40, Import Guard PR #41, browser-test stability PR #42, browser E2E PR #43, and security/dependency automation PR #46 are merged. Current exact main is `a8ce50d55ce826da751ddf856f73c31f2cc0a8a1`; exact-main CI `31816169303`, CodeQL `31816169327`, and Pages `31816169298` (build/deploy jobs `94818261669` / `94818542688`, deployment `5909115689`) passed with zero job annotations.
...
- Issue #45 local candidate adds a versioned, deterministic, local-only diagnostic JSON allowlist; counts are prepared only after an unlocked runtime flushes and re-reads the committed standard/encrypted snapshot. It excludes workspace names/IDs and all research/Zotero content, passphrases, and keys. Bilingual static GitHub feedback links, a sensitive-data warning template, `CITATION.cff`, `MAINTAINERS.md`, maintainer release checklist, and current-form Codex-for-OSS evidence/draft are included. The exact local candidate passed `npm ci`, official-registry audit with zero vulnerabilities, lint with zero warnings, typecheck, 37 files / 328 unit tests, the production/PWA build (1,977 modules; 25 precache entries / 1,430.45 KiB), 9 Playwright tests with 7 project-configured skips, 8 Zotero synthetic tests plus the reproducible 12,979-byte XPI hash, YAML parsing, and diff check. Remote PR/CI/self-review/merge/exact-main gates remain pending.
