...
> Status: [`v0.2.2`](https://github.com/Yoesher/sociology-phd-desk/releases/tag/v0.2.2) remains the latest formal Release. The authorized `v0.3.0` cycle is active: Phase A navigation and app-side Zotero/schema-v5 PR #39 are merged and exact-main verified; Zotero plugin PR #40 remains Draft pending its mandatory isolated Zotero 8 GUI gate; Issue #5 Import Guard is a local candidate. No `v0.3.0` Release is claimed.
...
### v0.3.0 Phase A local candidate — 2026-08-14

- Milestone `v0.3.0` exists. New Issues [#34](https://github.com/Yoesher/sociology-phd-desk/issues/34), [#35](https://github.com/Yoesher/sociology-phd-desk/issues/35), [#36](https://github.com/Yoesher/sociology-phd-desk/issues/36), and [#37](https://github.com/Yoesher/sociology-phd-desk/issues/37) represent navigation, Zotero import, the Zotero plugin, and the province-map compliance/feature gate. Existing Issues #5 and #6 are included; #2/#3/#4 remain future backlog.
...
- Zotero plugin 0.1.0 Draft PR #40 exact head `440f5fd8be541bd226020e97b6258bae3e505a2b` passes push/PR CI, six Node handoff tests, root 35 files / 314 tests, and reproducible XPI/checksum. It remains Draft because installation, selected/multi-item send, duplicate resend, fallback, disable/uninstall, and restart have not yet run in an isolated Zotero 8 GUI profile. An official signed Zotero 8.0.4 Windows installer was verified, but no GUI result is claimed.
- Issue #5 Import Guard local candidate uses a two-step write-free preview for portable JSON and encrypted backups, and the Zotero import retains the same bounded preview-before-write rule. It reports source/target versions, explicit migrations, counts, collection+ID collisions, relationship conflicts, and risks. Limits are 32 MiB portable JSON, canonical encrypted-wrapper maximum for 64 MiB ciphertext, 25,000 records per collection, 100,000 total, and the documented stricter Zotero thresholds. Final local checks pass: lint 0 warnings; typecheck; 36 files / 320 tests; Vite/PWA build with 1,976 modules and 25 static precache entries. Import Guard PR/remote/main/Pages and browser smoke remain pending.
