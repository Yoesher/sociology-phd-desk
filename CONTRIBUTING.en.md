...
1. Read [README.en.md](README.en.md), [PROJECT_STATE.md](PROJECT_STATE.md), [DECISIONS.md](DECISIONS.md), and [NEXT_TASKS.md](NEXT_TASKS.md).
...
- Treat Simplified Chinese as the default interface language and keep a complete English alternative. A substantial interface change must include both languages, including validation, accessible names, and responsive labels.
- Keep persisted domain values and portable JSON locale-neutral. Never translate or rewrite user-authored research content when the interface language changes.
- Use anonymous fieldwork identifiers and retain the participant-privacy warning.
- Stop any China Research Map hierarchy at county level and never add exact participant locations. Public map boundaries require documented authoritative provenance, permitted use, attribution, and versioning; an arbitrary dataset is not an acceptable substitute.
