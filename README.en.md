...
The current candidate reduces secondary-navigation complexity, consolidates status views into in-page filters, simplifies the top bar and workspace settings, and adds coherent, restrained motion for routes, dialogs, drawers, and state changes without changing research data or schema versions. It fully respects `prefers-reduced-motion` and adds no animation dependency.
